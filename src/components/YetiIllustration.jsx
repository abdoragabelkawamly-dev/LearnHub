...
const YetiIllustration = ({ inputSelectors = [] }) => {
  useEffect(() => {
    let gsap = null;
    let blinking = null;
    ...
    const calculateFaceMove = () => {
      ...
      if (!input || !inputSelectors.includes(`#${input.id}`)) return;
      if (!gsap) return;
      ...
      let carPos = input.selectionEnd;
      if (carPos == null || carPos === 0) carPos = input.value.length;
      ...
      div.textContent = input.value.substr(0, carPos);
      span.textContent = input.value.substr(carPos) || ".";
      div.appendChild(span);

      let targetX;
      if (input.scrollWidth <= emailScrollMax) {
      ...
      const tooth = document.querySelector(".tooth");
      const tongue = document.querySelector(".tongue");
      const eyeLEl = document.querySelector(".eyeL");
      const eyeREl = document.querySelector(".eyeR");
    ...
    const initYeti = () => {
      ...
      if (!mySVG || !firstInput) return;

      svgCoords = getPosition(mySVG);
      emailCoords = getPosition(firstInput);
      ...
      inputSelectors.forEach((selector) => {
        ...
        if (!el) return;
        const onFocus = () => calculateFaceMove();
        const onInput = () => calculateFaceMove();
        const onBlur = () => {
          setTimeout(() => {
            if (
              document.activeElement &&
              inputSelectors.includes(`#${document.activeElement.id}`)
            )
              return;
            resetFace();
            resetMouth();
        ...
        listeners.push({ el, event: "focus", fn: onFocus });
        listeners.push({ el, event: "input", fn: onInput });
        listeners.push({ el, event: "blur", fn: onBlur });
      });

      emailScrollMax = firstInput.scrollWidth;
      startBlinking(5);
    };
            ...
            >
              ...
              <g className="earR">
                ...
                <g className="earHair">
                  ...
                  <path
                    d="M146.6 62.8c4.9 4.6 8.4 9.4 10.6 14.2c-3.4-.1-6.8-.1-10.1.1 4 3.7 6.8 7.6 8.2 11.6-2.1 0-4.2 0-6.3.2 2.6 4.1 3.8 8.3 3.7 12.5-1.2-.7-3.4-1.4-5.2-1.9"
                    fill="#fff"
                    stroke="hsl(174 72% 32%)"
                    strokeWidth="2.5"
                    strokeLinecap="round"
...
export default YetiIllustration;
